...
module.exports = class RoutingGraphCMD {
    ...
    buildSavings() {
        ...
        this.matrix.forEach((c) => {
            if (c.get("type") != "otherschool") {
                let ckey = c.get("key");
                let cedges = this.useSpatialDistance ? c.get("spatialDistEdges") : c.get("spatialCostEdges");

                // For each neighbor d from c compute the saving Scd
                this.matrix.forEach((d) => {
                    let dkey = d.get("key");
                    let dtype = d.get("type");
                    if (ckey != dkey && ckey != "garage" && dkey != "garage" && ckey != "school" && dkey != "school" && dtype != "otherschool") {
    ...
    getSpatialDistance(c, d, spatialiteDB) {
        ...
        // Check if in cache
        if (
            this.cachedODMatrix.dist[crawkey] &&
            this.cachedODMatrix.dist[drawkey] &&
            (drawkey in this.cachedODMatrix.dist[crawkey] || crawkey in this.cachedODMatrix.dist[drawkey])
        ) {
            let dist = this.cachedODMatrix.dist[crawkey][drawkey] || this.cachedODMatrix.dist[drawkey][crawkey];
            //let cost = this.cachedODMatrix.cost[crawkey][drawkey] || this.cachedODMatrix.cost[drawkey][crawkey];

...
function validaDadosEscolhidos() {
    ...
    if (alunos.length == 0) {
        errorFn(
            `Não é possível realizar a sugestão de rotas. Para esta combinação de parâmetros não há nenhum aluno georeferenciado`,
            "",
...
function replotaDados() {
    ...
    let alunosFiltrados = [...alunoMap].filter((a) => {
        ...
        if (filtroSemRota) {
            alunoFiltrado = a[1]["GPS"] && a[1]["TURNO"] == filtroTurno; // TODO: Tem ponto de parada? && a[1]["TEM_ROTA"] == false;
        } else {
            alunoFiltrado = a[1]["GPS"] && a[1]["TURNO"] == filtroTurno;
        }
